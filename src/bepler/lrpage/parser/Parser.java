...
public class Parser<V> {
	...
	public Parser(Map<State<V>, Map<Class<? extends Terminal<V>>, Action<V>>> actionTable, State<V> startState){
		m_Actions = actionTable;
		m_Start = startState;
	}
	
	public Terminal<V> parse(List<Terminal<V>> tokens){
		Deque<State<V>> states = new LinkedList<State<V>>();
		states.push(m_Start);
		Deque<Terminal<V>> symbols = new LinkedList<Terminal<V>>();
		Deque<Terminal<V>> remaining = new LinkedList<Terminal<V>>(tokens);
		while(true){
			State<V> state = states.peek();
			Terminal<V> lookahead = remaining.peek();
	...
	public Terminal<V> parse(Lexer<V> lexer){
		Deque<State<V>> states = new LinkedList<State<V>>();
		states.push(m_Start);
		Deque<Terminal<V>> symbols = new LinkedList<Terminal<V>>();
		Deque<Terminal<V>> lookaheadStack = new LinkedList<Terminal<V>>();
		lookaheadStack.add(lexer.nextToken());
		while(true){
			Terminal<V> lookahead = lookaheadStack.peek();
			State<V> state = states.peek();
			Action<V> action = this.getAction(state, lookahead);
	...
	private static <V> String stackToString(Iterable<Terminal<V>> stack){
		StringBuilder b = new StringBuilder();
		b.append("[");
		boolean first = true;
		for(Terminal<?> s : stack){
			if(first){
				first = false;
			}else{
				b.append(", ");
	...
	private Terminal<V> reduce(Deque<Terminal<V>> stack, Deque<State<V>> states, Production<V> rule){
		List<Terminal<V>> lhs = new ArrayList<Terminal<V>>();
	...
	public Action<V> getAction(State<V> state, Terminal<V> symbol){
		return this.getAction(state, symbol.getSymbolType());
	}
	
	public Action<V> getAction(State<V> state, Class<? extends Terminal<V>> symbolType){
